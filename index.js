...
const GEMINI_CHAT_API_BACKUP_2 = process.env.GEMINI_CHAT_API_BACKUP_2 || '';
const GEMINI_CHAT_API_BACKUP_3 = process.env.GEMINI_CHAT_API_BACKUP_3 || '';
const GEMINI_CHAT_API_BACKUP_4 = process.env.GEMINI_CHAT_API_BACKUP_4 || '';
const GEMINI_CHAT_API_BACKUP_5 = process.env.GEMINI_CHAT_API_BACKUP_5 || '';
const GEMINI_CHAT_API_BACKUP_6 = process.env.GEMINI_CHAT_API_BACKUP_6 || '';
const GEMINI_CHAT_API_BACKUP_7 = process.env.GEMINI_CHAT_API_BACKUP_7 || '';
...
// Generate AI response using Gemini 2.5 Flash Lite with multiple backup APIs
async function generateAIResponse(message, telegramId, isFirstMessage = false) {
    ...
    const languageInstruction = userLanguage === 'english' 
        ? 'Respond in pure English only, no Hindi words.' 
        : 'Respond in natural Hinglish (mix of Hindi and English), just like chatting on WhatsApp.';
    ...
    const fullPrompt = `${prompt}${conversationContext}\nCurrent user message: ${message}\n\n${languageInstruction} Respond as a caring girlfriend, keeping the conversation natural and remembering what we talked about:`;

    // Array of API keys to try in order
    const apiKeys = [
        GEMINI_CHAT_API,
        GEMINI_CHAT_API_BACKUP,
        GEMINI_CHAT_API_BACKUP_2,
        GEMINI_CHAT_API_BACKUP_3,
        GEMINI_CHAT_API_BACKUP_4,
        GEMINI_CHAT_API_BACKUP_5,
        GEMINI_CHAT_API_BACKUP_6,
        GEMINI_CHAT_API_BACKUP_7
    ].filter(key => key && key !== 'YOUR_GEMINI_CHAT_API' && key !== 'YOUR_GEMINI_CHAT_API_BACKUP');
    ...
    for (let i = 0; i < apiKeys.length; i++) {
        try {
            ...
            if (response.data && 
                ...
                if (i > 0) {
                    console.log(`Success using backup API #${i}`);
                }
                return response.data.candidates[0].content.parts[0].text;
            } else {
                console.error(`Gemini API #${i + 1} returned invalid response structure:`, JSON.stringify(response.data).substring(0, 200));
            ...
            console.error(`Gemini API #${i + 1} failed:`, error.message);
            
            // If this was the last API key, return error message
            if (i === apiKeys.length - 1) {
                console.error('All Gemini APIs failed');
                return "Sorry baby, thoda problem hai. Phir se try karo! 💕";
            }
...
bot.action('agree_terms', async (ctx) => {
    try {
        ...
        await ctx.reply(
            ...
            `Choose your preferred language for chatting:`,
            {
                parse_mode: 'Markdown',
                ...Markup.inlineKeyboard([
                    [Markup.button.callback('🇮🇳 Hinglish (Hindi + English)', 'lang_hinglish')],
...
bot.action(['lang_hinglish', 'lang_english'], async (ctx) => {
    try {
        const language = ctx.match[0] === 'lang_hinglish' ? 'hinglish' : 'english';
        await db.updateUser(ctx.from.id, { language });
        
        await ctx.deleteMessage();
        const channelUsername = '@aaliyahsupport';
        
        await ctx.reply(
            `✨ Great choice! Now join our updates channel to continue:\n\n` +
            `📢 Channel: ${channelUsername}\n\n` +
            `Join the channel and click "I have joined" below:`,
            {
                ...Markup.inlineKeyboard([
                    [Markup.button.url('📢 Join Updates Channel', `https://t.me/aaliyahsupport`)],
                    [Markup.button.callback('✅ I have joined', 'verify_channel')]
                ])
            }
        );
    } catch (error) {
...
bot.action('verify_channel', async (ctx) => {
    try {
        // User clicked "I have joined", proceed to menu
        await ctx.deleteMessage();
        const aiResponse = await generateAIResponse("User just completed setup and started the bot", ctx.from.id, true);
        await ctx.reply(
            `${aiResponse}\n\n` +
            `💕 *Your Daily Limits:*\n` +
...
bot.command('pic', async (ctx) => {
    try {
        ...
        console.error('Pic command error:', error);
        ctx.reply('Image send nahi ho rahi baby! 💕');
    }
});
...
bot.command('menu', async (ctx) => {
    try {
        ...
        const currentLanguage = user.language || 'hinglish';
        const languageEmoji = currentLanguage === 'hinglish' ? '🇮🇳' : '🇬🇧';
        ...
        await ctx.reply(
            ...
            `${languageEmoji} Language: ${currentLanguage === 'hinglish' ? 'Hinglish' : 'English'}\n\n` +
            `Choose what you want baby! 😘`,
            {
                ...
                ...Markup.inlineKeyboard([
                    ...
                    [Markup.button.callback(`${languageEmoji} Change Language`, 'change_language')]
                ])
            }
        );
    } catch (error) {
...
bot.action('change_language', async (ctx) => {
    try {
        ...
        await ctx.reply(
            ...
            `Current: ${currentLanguage === 'hinglish' ? '🇮🇳 Hinglish' : '🇬🇧 English'}\n\n` +
            `Select the language you want me to chat in:`,
            {
                parse_mode: 'Markdown',
                ...Markup.inlineKeyboard([
                    [Markup.button.callback('🇮🇳 Hinglish (Default)', 'set_language_hinglish')],
                    [Markup.button.callback('🇬🇧 English', 'set_language_english')],
                    [Markup.button.callback('◀️ Back to Menu', 'back_to_main')]
                ])
            }
        );
        ...
        console.error('Language selection error:', error);
        ctx.reply('Language settings load nahi ho rahi baby! 💕');
    }
});

// Set language to Hinglish
bot.action('set_language_hinglish', async (ctx) => {
    try {
        await db.updateUser(ctx.from.id, { language: 'hinglish' });
        await ctx.answerCbQuery('Language set to Hinglish! 🇮🇳');
        await ctx.reply(
            `✅ Language changed successfully!\n\n` +
            `Ab main Hinglish mein baat karungi baby! 💕\n` +
            `(I'll chat in Hinglish - mix of Hindi and English)`,
            Markup.inlineKeyboard([
                [Markup.button.callback('◀️ Back to Menu', 'back_to_main')]
            ])
        );
        ...
        ctx.reply('Language change nahi ho rahi baby! 💕');
    }
});

// Set language to English
bot.action('set_language_english', async (ctx) => {
    try {
        await db.updateUser(ctx.from.id, { language: 'english' });
        await ctx.answerCbQuery('Language set to English! 🇬🇧');
        await ctx.reply(
            `✅ Language changed successfully!\n\n` +
            `I'll chat in pure English now baby! 💕`,
            Markup.inlineKeyboard([
                [Markup.button.callback('◀️ Back to Menu', 'back_to_main')]
            ])
        );
        ...
        console.error('Language update error:', error);
        ctx.reply('Language change failed baby! 💕');
    }
});
    ...
    res.send(`
        ...
        <body class="bg-gray-100 min-h-screen">
            <div class="container mx-auto px-4 py-8">
                <div class="bg-white rounded-lg shadow-lg p-6">
                    <div class="flex items-center justify-between mb-6">
                        <div>
                            <h1 class="text-3xl font-bold text-gray-800">AI Companion Bot - Admin Panel</h1>
                            <p class="text-sm text-gray-500 mt-1">Last updated: <span id="lastUpdate">Loading...</span></p>
                        </div>
                    ...
                    <div class="bg-gray-50 p-4 rounded-lg mb-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <button onclick="loadAllUsers(); autoRefreshUsers = true;" class="bg-gray-500 text-white px-4 py-2 rounded hover:bg-gray-600 mr-2">Load All Users</button>
                                <button onclick="loadBannedUsers(); autoRefreshUsers = false;" class="bg-red-500 text-white px-4 py-2 rounded hover:bg-red-600 mr-2">Load Banned</button>
                                <button onclick="refreshStats()" class="bg-green-500 text-white px-4 py-2 rounded hover:bg-green-600">Refresh Now</button>
                            </div>
                        </div>
            ...
            <script>
                ...
                // Auto-refresh users list if enabled
                setInterval(() => {
                    if (autoRefreshUsers) {
                        loadAllUsers();
                    }
    ...
    bot.launch()
        ...
        .catch((error) => {
            ...
            if (error.response?.error_code === 409) {
                console.log('⚠️  Another bot instance is already running with this token.');
                console.log('💡 Admin panel will continue running for user management.');
            } else if (error.response?.error_code === 401) {
                console.log('❌ Invalid BOT_TOKEN. Please check your token from @BotFather');
            } else {
                console.log('❌ Bot failed to start. Admin panel will continue running.');
            }
        });
